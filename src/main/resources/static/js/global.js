...
function mostrarMensaje(msg, ejecutarAccion){
	...
	$("body").append('<div class="modal fade" id="id_my_modal" role="dialog"><div class="modal-dialog"><div class="modal-content">' + 
					 '<div class="modal-header"><h4 class="modal-title">Mensaje</h4><button type="button" class="close" data-dismiss="modal">&times;</button></div>'+
...
function mostrarMensaje(msg, ejecutarAccion) {
    ...
    $("#id_btn_dialog_aceptar").click(function () {
        $('#id_my_modal').modal("hide");
        if (ejecutarAccion && typeof ejecutarAccion === 'function') {
            ejecutarAccion(); // Ejecutar la función proporcionada como parámetro
        }
        $('#modalUsuario').modal("hide");
        $('#modalPrestamistas').modal("hide");
        $('#myModalPago').modal("hide");
        $('#myModalRegistrarTargeta').modal("hide");
    });
